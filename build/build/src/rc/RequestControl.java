...
public class RequestControl 
{
	...
	private QueueEntry getNextQueueEntry()
	{
		...
		try {
			...
			ResultSet rs = DysMain.db.execRaw(sql);
	...
	private void calcVars()
	{
		...
		queueSize = DysMain.db.getQueueSize();
		...
		ReqMode reqMode = DysMain.db.getRequestMode();
		if(reqMode == ReqMode.OPEN)
			...
			percentFull = queueLength / DysMain.db.readRealParam("queueCloseMins");
		
//		 Write that stuff to the database
		try {
			DysMain.db.setParameter("queueLength", queueLength);
			DysMain.db.setParameter("queueOpen", queueOpen);
			DysMain.db.setParameter("percentFull", percentFull);
			DysMain.db.setParameter("queueSize", queueSize);
			DysMain.db.setParameter("currSongRemaining", DysMain.foobar.getCurrSongRemaining());
			DysMain.db.setParameter("currSongLength", currSongLength);
