...
public class QueueEntry
{
	...
	public static ObservableList<QueueEntry> getForwardQueue()
	{
		...
		try 
		{
			...
			ResultSet rs = DysMain.db.execRaw("SELECT * FROM " +RCTables.forwardQueueTable.getName()+ ";");
	...
	public void writeToDB()
	{
		Task<Void> tsk= new Task<Void>() {
			...
			protected Void call() throws Exception 
			{
				...
				{
					RCTables.forwardQueueTable.verifyExists(DysMain.db.getDb());
					PreparedStatement ps = DysMain.db.getDb().prepareStatement(sql);
					...
					DysMain.db.execRaw(ps); // write the statement down
					ps.close();
	...
	public void addToHistory(long time)
	{
		try {
			...
			//			
			//			PreparedStatement psSel = DysMain.db.getDb().prepareStatement(sqlSel);
			//			psSel.setString(1, song.getSongID());
			//			psSel.setLong(2, this.time);
			//			psSel.setString(3, vw.getUserID());
			//			
			//			ResultSet rsSel = DysMain.db.execRaw(psSel);
			//			
			...
			PreparedStatement psIns = DysMain.db.getDb().prepareStatement(sqlIns);
