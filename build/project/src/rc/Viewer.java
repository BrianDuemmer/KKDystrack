...
public class Viewer 
{
	...
	{
		RCTables.viewerTable.verifyExists(DysMain.db.getDb());
		ResultSet rs = null;
		...
		try 
		{
			...
			PreparedStatement ps = DysMain.db.getDb().prepareStatement("SELECT username, coins FROM " +RCTables.viewerTable.getName()+ " WHERE channelID=?;");
			ps.setString(1, userID);
			rs = DysMain.db.execRaw(ps);
	...
	public List<QueueEntry> getSongsPlayed(int maxMins)
	{
		...
		try {
			RCTables.viewerTable.verifyExists(DysMain.db.getDb());
			PreparedStatement ps = DysMain.db.getDb().prepareStatement("SELECT " // get everything, but make sure we know the order
					+ "song_name, "
					+ "ost_name, "
					+ "length, "
					+ "franchise_name, "
